...
export default function TransferTicketPage({ params }: { params: { ticketId: string } }) {
  ...
  const transferContext = {
    ...
    ticket: {
      ...
      currentOwner: ticketBundle.holder,
    },
  };
  ...
  const handleContinueTransfer = () => {
    ...
    window.setTimeout(() => {
      router.push(`/mis-boletos/${params.ticketId}/transfer/pending`);
    }, 2200);
  };
            ...
            <aside className="grid gap-6 xl:sticky xl:top-28">
              <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6 shadow-[0_24px_60px_rgba(0,0,0,0.24)]">
                ...
                <div className="mt-6 flex flex-col gap-3">
                  ...
                  <Link href={`/mis-boletos/${params.ticketId}`} className="inline-flex items-center justify-center gap-2 rounded-full border border-white/14 px-5 py-3 text-[0.86rem] font-medium tracking-[0.01em] text-zinc-200 transition hover:border-white/30 hover:text-white">
                    Cancelar
                  </Link>
                </div>
              </section>
