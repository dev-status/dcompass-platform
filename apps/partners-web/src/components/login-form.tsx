...
import { loginWithEmail } from "@dcompass/auth/client";
...
export function LoginForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const credentials = await loginWithEmail(getFirebaseEnv(), {
            ...
            <div className="space-y-3">
              <div className="space-y-2">
                <div className="flex items-center gap-3">
                  <Image src={brandAssets.mainLogo} alt="DCompass símbolo principal" width={44} height={44} className="h-10 w-10 rounded-xl object-contain" />
                  <div>
                    <Image src={brandAssets.lettersLogo} alt="DCOMPASS" width={160} height={34} className="h-5 w-auto" />
                ...
                <p className="max-w-2xl text-sm leading-relaxed text-zinc-300 sm:text-base">Entra para continuar con la operación de tu cuenta y tus eventos.</p>
              </div>
            </div>
              ...
              <div className="space-y-8">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="max-w-md text-sm leading-relaxed text-zinc-300">Usa tu cuenta para entrar al panel de trabajo de DCompass Partners.</p>
                  </div>
                </div>

                <form className="space-y-5" noValidate onSubmit={handleSubmit}>
                  <div className="space-y-2">
                    <label htmlFor="email" className="text-sm font-medium text-zinc-200">Correo electrónico</label>
                    <input
                      id="email"
                      type="email"
                    ...
                    />
                    {showError(validation.errors.email) ? <p id="email-error" className="inline-flex items-center gap-2 text-sm text-red-300"><FiAlertCircle className="text-sm" />{validation.errors.email}</p> : null}
                  </div>

                  <div className="space-y-2">
                    <div className="flex items-center justify-between gap-4">
                      <label htmlFor="password" className="text-sm font-medium text-zinc-200">Contraseña</label>
                      <span className="text-sm" style={{ color: colors.accent }}>Olvidé mi contraseña</span>
                    </div>
                    <div className="relative">
                      ...
                      />
                      <button type="button" onClick={() => setShowPassword((prev) => !prev)} className="absolute inset-y-0 right-0 inline-flex items-center px-4 text-zinc-400 transition hover:text-white" aria-label={showPassword ? "Ocultar contraseña" : "Mostrar contraseña"}>
                        {showPassword ? <FiEyeOff className="text-lg" /> : <FiEye className="text-lg" />}
                      </button>
                    </div>
                    {showError(validation.errors.password) ? <p id="password-error" className="inline-flex items-center gap-2 text-sm text-red-300"><FiAlertCircle className="text-sm" />{validation.errors.password}</p> : null}
                  </div>

                  {formError ? <div className="rounded-2xl border border-red-400/40 bg-red-500/10 px-4 py-3 text-sm text-red-200">{formError}</div> : null}
